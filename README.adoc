...
+
```bash
echo http://$(oc get route istio-ingress -o jsonpath='{.spec.host}{"\n"}' -n istio-system)/breaker
```
+
You should see an error page, since the application route is not yet exposed.
+
. Apply the `RouteRule` to direct traffic to the greeing service:
+
```bash
